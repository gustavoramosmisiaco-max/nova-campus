...
export default function RegistroConducta() {
  ...
  useEffect(function () {
    if (aulaSel && areaSel) cargarEstudiantes()
    else setEstudiantes([])
  }, [aulaSel, areaSel])
  ...
  async function seleccionarEstudiante(est) {
    ...
    const result = await supabase
      ...
      .select('*, docente:profiles!conductas_registro_registrado_por_fkey(full_name)')
                    ...
                    <div className="flex justify-between items-start gap-3">
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                          {new Date(h.fecha + 'T00:00:00').toLocaleDateString('es-PE')} · Registrado por {h.docente?.full_name || 'Docente'}
                        </p>
                      </div>
                      {h.imagen_url && (
