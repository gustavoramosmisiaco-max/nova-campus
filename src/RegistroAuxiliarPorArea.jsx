...
const NIVELES_INFO = [
  { letra: 'AD', nombre: 'Logro destacado', color: '#2563EB', bg: '#DEEBF7' },
  { letra: 'A', nombre: 'Logro esperado', color: '#16A34A', bg: '#E7F3E4' },
              ...
              <tr>
                {competenciasData.map(function (comp) {
                  return comp.capacidades.flatMap(function (cap) {
                    const span = Math.max(cap.instancias.length, 1)
                    return [
                      <td key={cap.id} colSpan={span} className="p-1.5 text-center" style={{ backgroundColor: '#E7F3E4', color: '#16A34A', border: '1px solid #E5E9F0', fontSize: 11, minWidth: 100 * span }}>
                        {cap.nombre}
                      </td>,
                      <td key={cap.id + '_promcap'} rowSpan={4} className="p-1 text-center font-semibold" style={{ backgroundColor: '#F4F6F9', color: '#16A34A', border: '1px solid #E5E9F0', fontSize: 10, minWidth: 28, verticalAlign: 'middle' }}>
            ...
            <div key={comp.id} className="bg-white rounded-xl p-4" style={{ border: '1px solid #E5E9F0' }}>
              <div className="flex justify-between items-center mb-2 flex-wrap gap-1">
                ...
                <p className="text-xs text-slate-400">
                  <span style={{ color: '#16A34A' }}>{aprobados} aprobados ({pctAprob}%)</span>
                  {' · '}
              ...
              <div className="w-full h-2.5 rounded-full overflow-hidden flex" style={{ backgroundColor: '#F4F6F9' }}>
                <div style={{ width: `${pctAprob}%`, backgroundColor: '#22C55E' }} />
